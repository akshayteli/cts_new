...
function load_table() {
    ...
    $.ajax({ 
            type: 'GET', 
            url:customurl+'datadisplay.php', 
            data: { get_param: 'value' }, 
            dataType: 'json',
            success: function (prr_priceData, status) {
                        create_table_prr(prr_priceData); 
...
function create_table_prr(prr_priceData) {
    var dashboard_area = '<table id="openIND-table" class="table table-bordered table-hover table-green dataTable" aria-describedby="openIND-table_info"><thead><tr role="row"><th>Indent No.</th><th>DO No.</th><th>LR No.</th><th>Vehicle No.</th><th>Created Date</th></tr></thead></table>';
    $("#openIND-table_wrapper").append(dashboard_area);
    for (var x=0;x<prr_priceData.length;x++) {

        var ind_no = prr_priceData[x].indent_no;
        var v_no = prr_priceData[x].vehicle_no;
        var l_no = prr_priceData[x].lr_no;
        var l_date = prr_priceData[x].lr_date;
        var do_no='';
        var recent_data = '<tbody><tr><td class="ind" id='+ind_no+'>'+ind_no+'</td><td>'+do_no+'</td><td>'+l_no+'</td><td>'+v_no+'</td><td>'+l_date+'</td></tr></tbody>';
