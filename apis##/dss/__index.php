...
function loginCheck(){
	...
	if($access){
		...
		//$userid = singlefield("ID","auth_user","email='".$username."' and password='".md5($password)."' and  admin_type_id=".$uType);
		// $userid = singlefield("id","auth_user","username='".$username."' and password='".md5($password)."' and active=1");

		// if($userid){
			
		// 	$_SESSION['NAME'] = singlefield("name","auth_user","id=".$userid);
		// 	$_SESSION['USERNAME'] = singlefield("username","auth_user","id=".$userid);

		// 	$_SESSION['USER_ID'] = $userid;
		// 	$_SESSION['ROLE'] = singlefield("role","auth_user","id=".$userid);
			...
			// setcookie("total_username", $cookie_user_name, $expTime, "/");
			// setcookie("total_password", $cookie_password, $expTime, "/");
			// setcookie("total_check", $sign_remember, $expTime, "/");
			//setcookie("total_uType", $uType, $expTime, "/");
			//echo "hi";

			// $arr = array('status' => 1,'user_type' => singlefield("role","users","user_id=".$userid), 'message' => 'Login Success. Redirecting...', 'name' => $_SESSION['NAME'], 'username' => $_SESSION['USERNAME'], 'img_url'=> singlefield("url","users","user_id=".$userid), 'user_id'=> $userid);
		// 	$arr = array('status' => 1,'user_type' => singlefield("role","auth_user","id=".$userid), 'message' => 'Login Success. Redirecting...', 'name' => $_SESSION['NAME'], 'username' => $_SESSION['USERNAME'], 'img_url'=> singlefield("url","auth_user","id=".$userid));
...
function moreInfoAudits(){
	...
	$fields = "audit_id,auditor_id,dealer_id,datetime";
	$table = "dss_audits";
	...
	//*************** For Process ********************
		...
		$dealer_score1 += getSummage(5,$audit_id);
		$dealer_score1 += getSummage(13,$audit_id);
		$dealer_score1 += getSummage(6,$audit_id);

		
		//This is for Calculating Repair SOP calculations
		$dealer_score1 += -calcRSOP($audit_id);



		//$dealer_score = $dealer_score1;
		$dealer_score = $dealer_score - $dealer_score1;
	...
	//************* For infrastructure ****************88
	    $dealer_score1 = singlefield("sum(dealer_score)","dss_answers_customers","audit_id=".$audit_id." and dealer_score>=1");
	    $dealerCount = singlefield("count(answer_id)","dss_answers_customers","audit_id=".$audit_id." and dealer_score>=1");
			
		if($dealerCount == 0) {
			$dealerCount = 1;
		}
		
		$dealer_score += ($dealer_score1/$dealerCount);
...
// function getDealers(){
	// 	$fields = "a.dealer_id, a.user_id, b.first_name, b.last_name";
	// 	$table = "gm_dealer a, auth_user b";
	// 	$cond = "a.user_id = b.id";
	// 	$rows = selectrec($fields,$table,$cond);
	// 	$str1 ='{"today_date":"'.date("m.d.y").'","dealers":[';
	// 	foreach($rows as $row){ 
	// 		$str1 .='{"dealer_id":"'.$row[1].'","dealer_code":"'.$row[0].'","dealer_name":"'.$row[2].'"},';
	// 	}
...
function startNewAudit(){
	...
	$table = "dss_audits";
	$fields = "auditor_id,dealer_id";

	$value = $_SESSION['USER_ID'].",".$txtDealer;
...
function fetchQuestions(){
    ...
    foreach ($rows as $row) {
		...
		//For Averageing
			...
			// 	$total_score = singlefield("sum(max_score)","dss_questions","active=1 and process_id =".$row[0]." and question_name NOT LIKE '%Customer 2:%'");


			// 	$questionIDS = "";
			// 	$rows1 = selectrec("question_id","dss_questions","active=1 and process_id =".$row[0]);
			// 	foreach ($rows1 as $row1) {
			...
			// 	$dealer_score = singlefield("sum(dealer_score)","dss_answers","active=1 and audit_id=".$auditID.$qStr);

			// 	$questionIDS = "";
			// 	$rows1 = selectrec("question_id","dss_questions","active=1 and process_id =".$row[0]." and question_name LIKE '%Customer 1:%'");
			// 	foreach ($rows1 as $row1) {
			...
			// 	$dealer_score1 = singlefield("sum(dealer_score)","dss_answers","active=1 and audit_id=".$auditID.$qStr);


			// 	$questionIDS = "";
			// 	$rows1 = selectrec("question_id","dss_questions","active=1 and process_id =".$row[0]." and question_name LIKE '%Customer 2:%'");
			// 	foreach ($rows1 as $row1) {
			...
			// 	$dealer_score2 = singlefield("sum(dealer_score)","dss_answers","active=1 and audit_id=".$auditID.$qStr);



			// 	$dealer_score = $dealer_score + (($dealer_score1+$dealer_score2)/2);
		...
		if($row[0]==12){
			...
			// if($submitted=="1"){
				$savedCount_1 = count(getModelIDs($auditID,5));

				if($savedCount_1>1)
					$dealer_score = $dealer_score/$savedCount_1;
			// }
				// echo "select sum(dealer_score) from dss_answers where audit_id=".$auditID.$tempStr;
				// echo $dealer_score;
			...
			// if($submitted=="1"){
				$fields_1 = "sub_heading_id,sub_heading_name";
				$table_1 = "dss_sub_heading";
				$cond_1 = "active=1 and process_id=6";
				$rows_1 = selectrec($fields_1,$table_1,$cond_1);


				$savedCount_1 = 0;
				foreach($rows_1 as $row_1){ 

					if(in_array($row_1[0],getHeaderIDs($auditID,6))){
						$savedCount_1++;
					}
			...
			// print_r(getModelIDs($auditID,13));

			// if($submitted){
				
				$savedCount_1 = count(getModelIDs($auditID,13));
...
function getQuestionsAnswers($typeID="",$row,$auditID,$sub_heading_id="",$modelID=""){
    ...
    //$headings = selectrec("sub_heading_id","dss_questions","active=1 and process_id=".$row[0].$newCond1.$groupBy);
    	...
    	//foreach ($headings as $heading) {
			...
			$str .= '{
					"technician_name":"'.singlefield("technician_name","dss_answers_technician_details", "active=1 ".$newCond1." and process_id=".$row[0]." and audit_id=".$auditID).'",
					"comments":"'.singlefield("other_details","dss_answers_comments", "active=1 ".$newCond1." and process_id=".$row[0]." and audit_id=".$auditID).'",
					"questions": [ ';
			//echo $newCond1;
	        ...
	        foreach ($rows2 as $row2) {
	            ...
	            if($answerID){
	            	$str1 .='"answer":"'.singlefield("dealer_score",$table1,"answer_id=".$answerID).'",
	            			"isNA":"'.singlefield("nafield",$table1,"answer_id=".$answerID).'",
	            			';
	            }
	            else{
	            	$str1 .='"answer": "",
...
function answersList(){
	...
	if(isset($_POST["technicianName"])){
		...
		$cond = "audit_id=".$auditID." and process_id = ".$processID." and model_id = ".$modelID." and sub_heading_id = ".$subHeadingID;
		// echo $cond;
		...
		if(selectrec("t_id",$table,$cond)){
			...
			$fields = "audit_id, process_id, model_id, sub_heading_id, technician_name";
			$value = $auditID.", ".$processID.", ".$modelID.", ".$subHeadingID.", '".$technicianName."'";
			insertrec($table,$fields,$value);
		}
	}
	...
	if(isset($_POST["commentsArea"])){
		...
		if(selectrec("o_id",$table,$cond)){
			...
			$fields = "audit_id, process_id, model_id, sub_heading_id, other_details";
			$value = $auditID.", ".$processID.", ".$modelID.", ".$subHeadingID.", '".$commentsArea."'";
			insertrec($table,$fields,$value);
		}
	}
	...
	$fields = "question_id, dealer_score, audit_remarks, audit_id, nafield";
	$chkfields = "question_id, audit_id";
	...
	foreach ($questionIDs as $questionID) {
		...
		if($answerID){
			...
			if($dealerScores[$count]=="NaN"){
				$fields = "question_id, audit_remarks, audit_id, nafield";
				$value = $questionID.",'".$audit_remarks."',".$auditID.",".$na_field;
			}
			else{
				$dScore = $dealerScores[$count];
				$value = $questionID.",".$dScore.",'".$audit_remarks."',".$auditID.",".$na_field;
			}
...
function answersList1(){
	...
	foreach ($questionIDs as $questionID) {
		...
		if($answerID){
			...
			$value = $questionID.",".$dealerScores[$count].",'".htmlspecialchars($auditRemarks[$count],ENT_QUOTES)."',".$auditID;
			insertrec($table,$fields,$value);
...
function submitQuestionnaire(){
	...
	if( updaterecs($table,$col_val, $cond)){
		if( (singlefield("pro_status",$table,$cond)==1) and (singlefield("man_status",$table,$cond)==1) and (singlefield("too_status",$table,$cond)==1) and (singlefield("inf_status",$table,$cond)==1)){
			...
			updaterecs($table,"status = 1", $cond);
...
function getSubHeaders($pid,$audit_id,$editable){
	...
	$fields = "sub_heading_id,sub_heading_name";
	$table = "dss_sub_heading";
	$cond = "active=1 and process_id=".$pid." and sub_heading_id in(".$SHIds.")";
	$rows = selectrec($fields,$table,$cond);
	...
	foreach($rows as $row){ 
		...
		$qIds = singlefield("GROUP_CONCAT(question_id)","dss_questions","sub_heading_id in(".$row[0].") and sub_heading_id!=0 and process_id=".$pid);
			...
			$str .= '{
				...
				foreach($rows2 as $row2){ 
					...
					$str .= '{
						...
						"remarks":"'.singlefield("audit_remarks","dss_answers_sub_category","active=1 and question_id=".$row2[0]." and audit_id=".$auditID).'"
						},';
...
function getModelIDs($audit_id,$pid){
...
function getHeaderIDs($audit_id,$pid){
	$qids = selectrec("question_id","dss_answers","audit_id=".$audit_id);
	$questionIDs = "";
	foreach ($qids as $qid) {
		$questionIDs .= $qid[0].",";
...
function getManpower(){
	...
	if($dt){		
		$arr = array('status' => 1, 'AMSV' => $dt[0], 'AMPV' => $dt[1], 'row_1' => $dt[2], 'row_2' => $dt[3], 'row_3' => $dt[4], 'row_4' => $dt[5], 'row_5' => $dt[6], 'row_6' => $dt[7], 'row_7' => $dt[8], 'row_8' => $dt[9], 'row_9' => $dt[10], 'row_10' => $dt[11], 'row_11' => $dt[12], 'row_12' => $dt[13], 'row_13' => $dt[14], 'row_14' => $dt[15], 'row_15' => $dt[16], 'audit_id' => $dt[17], 'total_dealer_score' => $dt[18]);
	}
	else{
...
function getAsms(){
	...
	if(isset($_GET["zsm_id"])){
		$userIDs = selectrec("user_id, zsm_id, id, area","gm_areaservicemanager","zsm_id='".$_GET["zsm_id"]."'");
	}
	else{
		// echo "1";
		$userIDs = selectrec("user_id, zsm_id, id, area","gm_areaservicemanager");
	}
...
/* ************************** Others ***********************************/
function calcRSOP($audit_id){

	$questionIDS = "";
	$rows1 = selectrec("question_id","dss_questions","process_id =6");
	foreach ($rows1 as $row1) {
	...
	$fields = "sub_heading_id,sub_heading_name";
	$table = "dss_sub_heading";
	$cond = "active=1 and process_id=6";
...
function fetchAuditData($fields,$table,$cond,$txtDealer,$audit_id){
	...
	foreach ($rows as $row) {
		...
		$aprocessIDs = selectrec("process_id","dss_process","dss_id=".$row[0]);
		$processIDs = "";
		...
		$questionIDS = "";
		$rows1 = selectrec("question_id","dss_questions","process_id in(".$processIDs.")");
		foreach ($rows1 as $row1) {
		...
		$questionIDS = substr($questionIDS,0,-1);
		$max_score = singlefield("sum(max_score)","dss_questions","process_id in(".$processIDs.")");


		if($row[0]==1){

			$rows_totals = selectrec("process_id,process_name","dss_process","active=1 and dss_id=".$row[0]);
			$max_score = 0;
			foreach ($rows_totals as $rows_total) {
				...
				else if($rows_total[0]==12){
					$total_score = "20";

				}
				else if($rows_total[0]==5){
					$total_score = "100";

				}
				else if($rows_total[0]==6){
					$total_score = "70";

				}
				else if($rows_total[0]==13){
					$total_score = "40";
				}
				else{
					$total_score = singlefield("sum(a.max_score)","dss_questions as a, dss_process as b","a.active=1 and a.process_id=b.process_id and a.process_id = ".$rows_total[0]);
				}
		...
		if($questionIDS!=""){
			$dealer_score = singlefield("sum(dealer_score)","dss_answers","audit_id=".$audit_id." and question_id in(".$questionIDS.")");
		...
		$subCategory = singlefield("count(dss_sub_id)","dss_sub_categories","dss_id=".$row[0]);
		...
		if($subCategory==0)
			...
			if($row[0]=="0"){
		    	$subStat = singlefield("status","dss_audits","audit_id=".$audit_id);
		   }

		   if($row[0]=="1"){
		    $subStat = singlefield("pro_status","dss_audits","audit_id=".$audit_id);   
		   ...
		   if($row[0]=="2"){
		    $subStat = singlefield("man_status","dss_audits","audit_id=".$audit_id);
		    $dealer_score = singlefield("total_dealer_score","dss_answers_manpower","audit_id=".$audit_id);

		   }

		   if($row[0]=="3"){
		    $subStat = singlefield("too_status","dss_audits","audit_id=".$audit_id); 
		    $max_score = singlefield("sum(sc_question_marks)","dss_sub_category_question");
		    $dealer_score = singlefield("sum(dealer_score)","dss_answers_sub_category","audit_id=".$audit_id);
		   }

		   if($row[0]=="4"){
		    $subStat = singlefield("inf_status","dss_audits","audit_id=".$audit_id); 
		    $dealer_score = singlefield("sum(dealer_score)","dss_answers","audit_id=".$audit_id." and question_id in(".$questionIDS.")");
		   }
		   if($subStat!="1")
    			$subStat = 0;

		$ds = ($dealer_score>=1)?(round(($dealer_score/$max_score)*100,2))."%":"";
...
function getreportAPI(){
    ...
    foreach ($rows as $row) {
		...
		if($row[0]==12){
			...
			//if($submitted=="1"){
				$savedCount_1 = count(getModelIDs($arow[0],5));
			...
			// if($submitted=="1"){
				...
				foreach($rows_1 as $row_1){ 
					...
					if(in_array($row_1[0],getHeaderIDs($arow[0],6))){
						$savedCount_1++;
					}
			...
			// if($submitted){
				...
				$savedCount_1 = count(getModelIDs($arow[0],13));
	 ...
	 //        "has_text": "'.$hasTxt.'"';
		...
		$str2 .= '],
			...
			"grand_total_details":["'. round(($g_total / 1000)*100,2) .'"] 
		},';
					...
					<table style="background: '.$arrColor[$count].' none repeat scroll 0px 0px; color: rgb(239, 239, 239);" width="100%">
						<tr>
							<td align="center" style="font-size:12px;">
								...
								<p>Weightage : '.round($max_score,2).'</p>
